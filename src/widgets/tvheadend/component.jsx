...
export default function Component({ service }) {
  ...
  console.log("Widget Limit:", widget?.limit);
  console.log("Widget Object:", widget);

  const params = {
      limit: 10000, // Set limit high so all results are returned in 1 page
      refreshInterval: 60000,
    };

  const { data: dvrData, error: dvrError } = useWidgetAPI(widget, "dvr", params);
  ...
  const finishedCount = dvrData.entries.filter((entry) => entry.sched_status === "completed" && entry.status === "Completed OK").length;
